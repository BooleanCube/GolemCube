    ...
    public void run(List<String> args, GuildMessageReceivedEvent event) {
        if(args.size() >= 1) {
            ...
            if(timeout.containsKey(event.getMember().getIdLong()) && timeout.get(event.getMember().getIdLong()) + 3600000 > System.currentTimeMillis()) {
                event.getChannel().sendMessage(new EmbedBuilder().setDescription("You can only use the `" + Constants.PREFIX + "rep` command once an hour!\nWait for " + Tools.secondsToTime((timeout.get(event.getMember().getIdLong()) + 3600000 - System.currentTimeMillis())/1000)).build()).queue();
                return;
            } else if(timeout.containsKey(event.getMember().getIdLong())) timeout.remove(event.getMember().getIdLong());
