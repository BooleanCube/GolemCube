        ...
        if(event.getMember().getRoles().contains(event.getGuild().getRoleById(773337238952083477L))) {
            try {
                ...
                if(args.size() == 1) {
                    ...
                    if(Tools.memberToWarns.containsKey(event.getGuild().getMemberById(id))) Tools.memberToWarns.get(event.getGuild().getMemberById(id)).add(new Warning("No reason provided!", System.currentTimeMillis()));
